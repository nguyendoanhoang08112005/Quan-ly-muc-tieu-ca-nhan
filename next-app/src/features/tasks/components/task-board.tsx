...
export function TaskBoard({ tasks }: { tasks: TaskListItem[] }) {
  ...
  const dropTargetStatusRef = useRef<WorkStatus | null>(null);
        ...
        <div className="grid min-w-[68rem] gap-3 xl:grid-cols-4">
          {taskColumns.map((column) => {
            ...
            return (
              <section
                className={cn(
                  "ui-board-column min-h-[calc(100vh-15rem)] p-3 transition",
                  ...
                  <div>
                    <span
                      className={cn(
                        ...
                        workStatusClassNames[column.status]
                      )}
                    >
                      {workStatusLabels[column.status]}
                    </span>
                    ...
                    columnTasks.map((task) => (
                      <article
                        className={cn(
                          ...
                          syncingTaskIds.includes(task.id) && "ring-1 ring-stone-300"
                        )}
                        draggable={!syncingTaskIds.includes(task.id)}
                        key={task.id}
                        onDragEnd={() => {
                          setDraggedTaskId(null);
                          setActiveDropTarget(null);
